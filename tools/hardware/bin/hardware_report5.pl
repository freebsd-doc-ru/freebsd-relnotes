...
#$hardware_file = "tools/hardware/hardware_delme.adoc";
...
my %man4_basename;  # относительный путь -> наличие секции HARDWARE
...
if (-d $man4dir) {
    find({
        wanted => sub {
            ...
            $man4{$relname} = 1;
            my $base = basename($name);             # "archive.tar.gz"

            # Удаляем последнее расширение (после последней точки),
            # но не трогаем скрытые файлы без других точек
            $base =~ s/\.[^.]*$// if $base =~ /\./ && $base !~ /^\./;

            #print STDERR "$name $relname $base\n";
            $man4_basename{$base} = $relname;
...
while (my $line = <$lf>) {
    ...
    #push @driver_list, { name => $driver_name, file => $file_path };
     $driver_list{$driver_name} = { name => $driver_name, file => $file_path };
}
close($lf);
}
...
printf "%-25s %-8s %-8s %-4s %-5s %-8s %-8s %-20s %s\n",
...
foreach my $drvkey (sort keys %driver_list) {
    my $drv = $driver_list{$drvkey};
    my $name = $drv->{name};
    my $file = $drv->{file};
    ...
    my $has_man4          = ($man4{$name} || (defined($man4_basename{$name}) && $man4{$man4_basename{$name}}) )          ? 1 : 0;
    my $has_hw_section    = ($man4_hardware_section{$name} || (defined($man4_basename{$name}) && $man4_hardware_section{$man4_basename{$name}})) ? 1 : 0;
    ...
    delete $man4{$name};
    delete $man4{$man4_basename{$name}} if defined $man4_basename{$name};

    printf "%-25s %-8s %-8s %-4s %-5s %-8s %-8s %-20s %s\n",
        substr($name . " " x 25, 0, 25),
