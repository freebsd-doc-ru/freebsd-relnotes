...
Partial support for the C23 version of the C Programming language is now available. It will continue to evolve, with complete support anticipated for FreeBSD 16.
...
The commit updates stdbool.h to use built-in bool, true, and false when a C23 or higher compiler is detected, ensuring conformance with the C23 standard. This may affect developers compiling code with C23 features.
gitref:5656d0f7453e[repository=src].

Adds char8_t and atomic_char8_t types for C23 compliance, affecting developers using C23 features.
gitref:72609bb2bb25[repository=src].

Adds C23 _WIDTH macros to stdint.h headers, providing standardized integer width definitions for C23-compliant code.
gitref:ff50ace5fe0e[repository=src].

The commit adds C23 _WIDTH macros to sys/limits.h for compliance with ISO/IEC 9899:2024, affecting developers using these standard integer width definitions.
gitref:cbad54ba46e6[repository=src].

The commit updates man pages for sinpi, cospi, and tanpi functions to document that they follow the C23 standard.
