...
The freebsd-update utility now installs shared libraries in a specific order (libsys, libc, libthr, then others) to prevent failures during upgrades from 14.x to 15.x.
...
The nuageinit tool now supports the chpasswd command, allowing password changes via a list or multiline string, including deprecated syntax for compatibility with some providers.
...
Spleen font updated to version 2.2.0, adding missing characters (em-dash, en-dash, hyphen, angle brackets, white square, dagger, double dagger) and improving character alignment, particularly for high-dpi displays.
...
libarchive updated to version 3.8.5. This includes a bug fix for bsdtar to resolve a regression in zero-length pattern handling.
gitref:39fd1181e5b2[repository=src].

xz has been updated to version 5.8.2.
gitref:07700b0107dc[repository=src].

mtree updated to version from NetBSD, improving compatibility and fixing bugs in man:mtree[8].
gitref:f9d671f726ac[repository=src].

pci_vendors database updated to version 2025-12-12.
gitref:f21385688f52[repository=src].

tzdata updated to version 2025c.
gitref:68e2f4cc5e4e[repository=src].

bmake updated to version 20251111.
gitref:c95f96dea30a[repository=src].

SQLite updated to version 3.50.4.
gitref:ef55f6b86626[repository=src].

libarchive updated to version 3.8.4. This includes fixes for bsdtar pattern handling and directory traversal issues.
gitref:d39dd181c8ab[repository=src].
...
libarchive updated to version 3.8.3. This includes bug fixes for temporary file creation location, buffer overruns in LHA and 7-Zip format handling, and support for LZ4 and Zstd data with leading skippable frames.
gitref:eb042d5e8de3[repository=src].

OpenZFS updated to version 2.2.9. This release includes improvements to ARC shrinking, fixes for 'zpool add' safety checks, zvol blk-mq synchronization, and BRT range conversion math.
gitref:709465f2c4f1[repository=src].

less updated to version 685.
gitref:054ae5e7b465[repository=src].

USB vendor database updated to 2025-09-15.
gitref:d565a5e904ed[repository=src].

Unbound updated to version 1.24.1. This release includes a security fix for CVE-2025-11411.
gitref:eeb41dca070f[repository=src].

The man:newaliases[1] man page is updated to clarify that it is for sendmail.
gitref:e3df9a78da6b[repository=src].

Unbound updated to version 1.24.0.
gitref:fe7bb59bb014[repository=src].
...
OpenSSL updated to version with fixes for CVE-2025-9230 and CVE-2025-9232.
gitref:270158508d7c[repository=src].

mandoc updated to version 2025-09-26. The update improves case sorting in mandoc db, adds macros for AT&T Unix versions 8 and 10, warns on blank lines in man(7) like mdoc(7), and fixes a PDF/PS footer regression.
gitref:7fa4ccb8e4e7[repository=src].

expat updated to version 2.7.3.
gitref:a85cfcb61efd[repository=src].

bc and dc updated to version 7.1.0.
...
libucl updated to version 0.9.2.
gitref:0a8d8b0c878f[repository=src].
{{< sponsored "The FreeBSD Foundation" >}}

mandoc updated to Groff Compat Edition with improved groff compatibility in formatting, error handling, and rendering.
gitref:8039d22f6afd[repository=src].

OpenSSH updated to version 10.0p2. The update removes support for the weak DSA signature algorithm and changes the default key agreement to the post-quantum hybrid algorithm mlkem768x25519-sha256. The sshd(8) authentication phase now runs in a separate sshd-auth binary.
...
mandoc updated to upstream version from 2025-06-13. The LIBRARY section is deprecated, and the Ft macro now accepts multiple arguments without quoting. A segmentation fault on malformed Tg markup is fixed.
gitref:d0b23791c667[repository=src].

libarchive updated to version 3.8.1. This includes security fixes for multiple CVEs, new zip writer compression support for XZ, LZMA, ZSTD and BZIP2, and bsdtar gains --mtime and --clamp-mtime options.
gitref:6dad4525a291[repository=src].
...
bmake updated to version 20240711.
gitref:fa86ef7f23ae[repository=src].
...
The qat(4) driver now supports the 402xx device (IDs 0x4944/0x4945) under the existing qat_4xxx driver, and adds required firmware files.
...
The ix and ixv drivers add support for the Intel Ethernet E610 family of devices, including new PCI IDs for backplane, SFP, 10 GbE, 2.5 GbE, and SGMII variants. This enables link speeds of 2.5G, 5G, and 10G on supported hardware.
...
The iwlwifi driver now includes ACPI support, enabling regulatory features for 802.11ax, 802.11be, and Per Platform Antenna Gain (PPAG) settings.
...
The tarfs filesystem now correctly handles large files exceeding 4 GB and 8 GB limits. It fixes decompression errors when seeking beyond 4 GB in zstd-compressed tarballs and properly processes extended header records for files larger than 8 GB.
...
Adds a new 9P filesystem implementation (p9fs) for use with bhyve virtio-9p devices. It allows guests to access host files via share mappings and can be used as a root or non-root filesystem. The driver is loaded via 'virtio_p9fs_load=YES' in loader.conf.
...
The bsdinstall installer now copies loader.efi to all ESPs created for multi-volume ZFS datasets, providing boot redundancy if the primary disk fails.
...
The initramfs zfs boot script now uses LVM autoactivation (`vgchange -aay`) instead of unconditional activation. This respects LVM autoactivation settings like --setautoactivation and auto_activation_volume_list, allowing users to control which logical volumes are activated at boot. Misconfigured setups with ZFS on top of an LV that has autoactivation disabled may no longer boot.
...
Removes obsolete compatibility code for IPFW versions prior to FreeBSD 8, simplifying the codebase. This may affect users or third-party modules that still rely on the old compatibility interfaces.
...
bhyve now reports SVM as disabled in the VM control register, preventing hangs on AMD systems with recent Windows guests.
